...
- 도형 분류기: 닫힘·모서리 판정, 사각형/타원, 최소 크기 거부, 화살표 미분류, 튜닝 상수 계약
...
7. 획 홀드 `450ms` — 닫힌 획은 사각형·타원으로 보정되고 열린 획은 시작점이 고정된 직선으로 잠김. 진행 링은 `150ms`부터 표시되고, 잠긴 뒤 포인터 이동은 끝점을 조절하며 <kbd>Shift</kbd>는 방향을 45° 단위로 제한. 홀드 없이 떼면 자유곡선 유지, 손으로 그린 화살표는 자동 보정되지 않음, 보정 후 <kbd>⌘Z</kbd> 한 번에 제거
