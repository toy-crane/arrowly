...
- 타이핑 텍스트: 신규 진입·기존 텍스트 hit testing·클릭 지점 캐럿·내용/크기 재편집·빈 값 삭제·확정/취소·IME 조합·편집 중 단축키 흡수
- 텍스트 이력: 추가·교체·삭제 Undo/Redo, 내용+크기 한 단위, no-op 편집 제외, 겹친 텍스트의 최상위 선택
...
2. <kbd>Enter</kbd>·바깥 클릭 확정 — 확정 클릭이 새 획을 시작하지 않고, 텍스트가 선택한 고정 크기로 렌더
3. 입력 중 <kbd>⌘Z</kbd> = 입력 전체 취소 / 첫 <kbd>Esc</kbd> = 현재 내용 확정·편집 종료 / 두 번째 <kbd>Esc</kbd> = 그리기 종료
4. 기존 텍스트 더블클릭 — 최상위 마크 선택, 클릭 글자 근처 캐럿, 원본 캔버스 중복 표시 없음, 위치·색 유지
...
8. 설정 창에 고정 ⌘+/⌘− 행, 온보딩 ③에 크기 변경·더블클릭 재편집 안내가 노출
9. 웹뷰 응답 중에는 첫 Esc로 편집 확정, 웹뷰를 멈춘 뒤에는 다음 Esc로 통과 모드 탈출
